...
def csv_cut_HeelStrike(csv_path, save_path):
    ''' csv_path = 원본 CSV 주소, 
        save_path = 저장 디렉토리
        return True
    ...
    try:
        ...
        for row in range(num_rows):
            ...
            if flag == False and heel_value > 0:
                ...
                file_name = "CSV_{:02d}".format(index) + ".csv"
                data_save_path = os.path.join(save_path, file_name)  # 파일 저장 경로 생성
...
# raw csv 시각화
def csv_to_plot_raw(data_root , save_root):
    ''' data_root = 원본 CSV 주소, 
        save_root = 저장 디렉토리
        return True
    ...
    try:
        ...
        plt.grid()
        save_png = os.path.join(save_root, "raw.png")
...
# spride csv 시각화
def csv_to_plot_spride(data_dir, save_root):
    ''' data_root = 원본 CSV 주소, 
        save_root = 저장 디렉토리
        return True
    ...
    try:
        for filename in os.listdir(data_dir):
            if filename.endswith(".csv"):
                ...
                plt.grid()
                save_png = os.path.join(save_root, f"{os.path.splitext(filename)[0]}.png")
                plt.savefig(save_png , dpi=300, bbox_inches='tight')
                plt.close()
...
def predict_model(sequence):
    ...
    try:
        model_path = "./modelRNN/model.h5"  # 학습 완료된 모델 경로
